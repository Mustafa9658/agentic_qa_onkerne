"""
Screenshot storage service for OnKernal agents.

Provides simple screenshot storage and retrieval functionality for:
- Judge evaluation (needs screenshot paths)
- GIF generation (needs screenshot history)
- Code-use agents (needs screenshot tracking)
- Execution history (needs visual trace)
"""
...
class ScreenshotService:
	"""Simple screenshot storage service that saves screenshots to disk for judge/GIF/history tracking"""
	...
	def __init__(self, agent_directory: str | Path):
		...
		Args:
			agent_directory: Directory where screenshots will be stored
		"""
	...
	async def get_screenshot(self, screenshot_path: str) -> str | None:
		...
		Returns:
			Base64 encoded screenshot or None if not found
		"""
