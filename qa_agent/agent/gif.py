...
from qa_agent.agent.views import AgentHistoryList
from qa_agent.browser.views import PLACEHOLDER_4PX_SCREENSHOT
from qa_agent.config import settings as CONFIG
...
def create_history_gif(
		...
		for font_name in font_options:
			try:
				...
				if platform.system() == 'Windows':
					font_path = os.path.join('C:/Windows/Fonts', font_name + '.ttf')
				elif platform.system() == 'Darwin':  # macOS
					font_path = font_name  # macOS finds fonts by name
				else:  # Linux and other Unix-like systems
					font_path = font_name  # Linux finds fonts by name via fontconfig
	...
	if show_logo:
		try:
			logo = Image.open('./static/browser.png')
