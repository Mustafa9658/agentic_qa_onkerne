...
if TYPE_CHECKING:
	from qa_agent.browser.session import BrowserSession, CDPSession
	...
	async def _handle_target_attached(self, event: AttachedToTargetEvent) -> None:
		...
		async with self._lock:
			...
			if target_id not in self.browser_session._cdp_session_pool:
				from qa_agent.browser.session import CDPSession

				# Check if root CDP client is available
				if self.browser_session._cdp_client_root is None:
					self.logger.warning(
						f'[SessionManager] Cannot create session for target {target_id[:8]}... - Root CDP client not available. '
						f'This may happen when new tabs are opened before browser is fully initialized.'
					)
					return  # Skip session creation if root client is not available
		...
		if waiting_for_debugger:
			try:
				# Check if root CDP client is available (graceful handling)
				if self.browser_session._cdp_client_root is None:
					self.logger.warning(
						f'[SessionManager] Cannot resume debugger - Root CDP client not available. '
						f'This may happen when new tabs are opened before browser is fully initialized.'
					)
				else:
	...
	async def _handle_target_detached(self, event: DetachedFromTargetEvent) -> None:
		...
		if target_fully_removed:
			if target_type in ('page', 'tab'):
				from qa_agent.browser.events import TabClosedEvent
	...
	async def _recover_agent_focus(self, crashed_target_id: TargetID) -> None:
		...
		try:
			...
			if all_pages:
				...
				from qa_agent.browser.events import TabCreatedEvent
			...
			if new_session:
				...
				if is_existing_tab:
					try:
						...
						if self.browser_session._cdp_client_root is None:
							self.logger.warning(
								f'[SessionManager] Cannot activate tab - Root CDP client not available. '
								f'This may happen when new tabs are opened before browser is fully initialized.'
							)
						else:
				...
				from qa_agent.browser.events import AgentFocusChangedEvent
			...
			for _ in range(20):
				...
				if fallback_session:
					...
					from qa_agent.browser.events import AgentFocusChangedEvent, TabCreatedEvent
